import {
  CandidateValidator,
  ValidationError,
} from '../../application/validators/CandidateValidator';
...
describe('CandidateValidator', () => {
  ...
  describe('email format', () => {
    ...
    it('passes for valid email', () => {
      ...
      expect(
        errors.filter((e: ValidationError) => e.field === 'email'),
      ).toHaveLength(0);
    });
  });
  ...
  describe('workExperience validation', () => {
    ...
    it('fails when company is missing', () => {
      const errors = validator.validate({
        ...validBase,
        workExperiences: [
          { company: '', position: 'Dev', startDate: '2022-01-01' },
        ],
      });
